...
A `dagger.json` defines a dagger module. If the module has a `src` field, the dagger module source is in the folder pointed to by the field.
...
2. Run `dagger develop` to regenerate module code.
